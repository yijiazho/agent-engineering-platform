...
explicitly versioned Workflow references, Workflow Task DAGs resolve into
validated deterministic execution plans with parallel-ready groups, and
...
repository-bound Git Tool now provides branch, status, diff, and authorized
push operations with redacted command evidence. The end-to-end MVP workflow is
not yet runnable.
